    ...
    with open(REFERENCE, encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            if header:
                ...
                # One entry reads "T2 T4 Akula Vector Dreadnought, Zilant" -- a
                # stray tier copied from the line above. The first tier is the
                # real one: every weapon and ability Zilant equips is a /T2/
                # asset. Strip the duplicate so the manufacturer still parses.
                midclass = re.sub(r"^T\d\s+", "", midclass.strip())
                current = {
                    "tier": int(tier[1:]),
                    "midclass": midclass,
                    "name": name.strip(),
...
def main():
    ...
    for entry in entries:
        is_hero = "/Hero/" in entry["path"]
        if "/Precast/" not in entry["path"] and not is_hero:
            continue
    ...
    hero_rows.sort(key=lambda r: (r["manufacturer"], r["tier"], r["name"]))
    from_client = sum(1 for r in rows if r["named_by_client"])
    hero_from_client = sum(1 for r in hero_rows if r["named_by_client"])
    missing_manufacturer = [r["name"] for r in hero_rows if not r["manufacturer"]]
    ...
    out.append("}")
    out.append("")
    out.append("// heroShipLoadout is a hero ship. It carries its own manufacturer because")
    out.append("// heroes do NOT follow the hull-line rule the base roster does -- the same")
    out.append("// reference shows Kore as Oberon and Tunmen as Jupiter Arms, both SniperHeavy.")
    out.append("//")
    out.append("// These belong in the tech tree document: the client does not fetch heroes")
    out.append("// separately. GetHeroShipsFromManufacturerData reads the manufacturer's ordinary")
    out.append("// item array and keeps the entries whose type byte the manager set to \"hero\",")
    out.append("// which it does purely from the id's category (3 = YShipLoadoutHero).")
    out.append("type heroShipLoadout struct {")
    out.append("\tloadoutID    int32")
    out.append("\thullLine     string")
    out.append("\ttier         int32")
    out.append("\tname         string")
    out.append("\tclientNamed  bool")
    ...
    out.append("}")
    out.append("")
    out.append(f"// {len(hero_rows)} hero ships; {hero_from_client} named by ItemIDConversionTable,")
    out.append(f"// {len(hero_rows) - hero_from_client} by the reference.")
    out.append("var heroShipLoadouts = []heroShipLoadout{")
    for r in hero_rows:
        out.append(
            "\t{loadoutID: %d, hullLine: %s, tier: %d, name: %s, clientNamed: %s, manufacturer: %s, "
            "primary: %d, secondary: %d, abilities: [4]int32{%s}, perks: [4]int32{%s}},"
            % (
                r["id"],
                json.dumps(r["line"]),
    ...
    if missing_manufacturer:
        print(f"WARNING: {len(missing_manufacturer)} heroes have no manufacturer: {missing_manufacturer}", file=sys.stderr)

    with open(OUTPUT, "w", encoding="utf-8") as handle:
        handle.write("\n".join(out))
    print(f"wrote {OUTPUT}: {len(rows)} hulls ({from_client} client-named), {len(hero_rows)} heroes ({hero_from_client} client-named)")
