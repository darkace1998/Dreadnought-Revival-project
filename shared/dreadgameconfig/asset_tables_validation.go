...
func (v *AssetTablesValidation) performCrossTableValidations() {
	...
	// There is deliberately no ItemIDRegister-to-CatalogIDTable check.
	//
	// One used to live here and reported 0/3086 matches on every boot, which is
	// what made the summary print "some asset table validations have
	// mismatches" every time the server started. It was not detecting a data
	// problem: the two tables use disjoint ID spaces by design. ItemIDRegister
	// is keyed by asset item IDs (33489185, 100597772, ...), while every
	// CatalogIDTable bucket holds storefront offer IDs in their own ranges
	// (Bundles 999300xx, Captain Vanity 999400xx, Coatings 999500xx, Modules
	// 99967xxxxxx, ...) plus an un_typed bucket of opaque string SKUs. No
	// offer ID in any bucket is an item ID, and stripping the shared 999 prefix
	// does not recover one either (1 of 3498 decode into the register, i.e.
	// coincidence). The check could therefore never pass, so it only ever
	// masked the two comparisons below that do mean something.

	// 2. Validate that every ItemIDConversionTable mapping lands on an ID the
	// ItemIDRegister can actually resolve to an asset path.
	//
	// This deliberately checks NewItemID, not OldItemID. Checking the old IDs
	// reported 0/1616 forever and could not have done anything else: the
	// conversion table exists precisely because those legacy IDs (1000001,
	// 1000002, ...) were retired, and the register only ever holds the current
	// IDs they were remapped to (33489313, ...). The migration target is the
	// end of the mapping that has to resolve, and checking it surfaces the
	// mappings that genuinely dangle.
	conversionEntries := GetAllItemIDConversionEntries()
	conversionNewIDs := make(map[int64]bool)
	for _, entry := range conversionEntries {
		conversionNewIDs[entry.NewItemID] = true
	}
	...
	matches = 0
	for newID := range conversionNewIDs {
		if registerItemIDsInt64[newID] {
			matches++
		}
	}
	...
	v.CrossTableValidationResults = append(v.CrossTableValidationResults, CrossTableValidationResult{
		...
		Table2:          "ItemIDRegister",
		ValidationType:  "New ID existence",
		TotalChecked:    len(conversionNewIDs),
		MatchesFound:    matches,
		MismatchesFound: len(conversionNewIDs) - matches,
		Details:         fmt.Sprintf("%d/%d conversion new IDs found in ItemIDRegister", matches, len(conversionNewIDs)),
	})
