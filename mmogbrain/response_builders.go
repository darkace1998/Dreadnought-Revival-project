...
func appendMmogOwnedShipLoadoutEntry(b []byte, stack []int, playerPID string, loadout mmogShipLoadoutSeed) ([]byte, []int) {
	...
	b = protocol.AppendStringField(b, "ID", loadout.entryID())
	b = protocol.AppendStringField(b, "PID", playerPID)
	b = protocol.AppendStringField(b, "LoadoutName", loadout.loadoutName)
	...
	b = protocol.AppendInt32Field(b, "shipID", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "ShipID", loadout.ship.id)
	b = protocol.AppendBoolField(b, "isActiveLoadout", false)
	b = protocol.AppendBoolField(b, "m_isActiveLoadout", false)
	b, stack = protocol.AppendObjectEnd(b, stack)
	return b, stack
}
...
func appendMmogLoadoutItemStateEntry(b []byte, stack []int, loadout mmogShipLoadoutSeed, slot mmogLoadoutItemSeed, state string) ([]byte, []int) {
	...
	b = protocol.AppendInt32Field(b, "LoadoutID", loadout.loadoutID())
	b = protocol.AppendInt32Field(b, "ShipID", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "ItemID", slot.itemID)
	b = protocol.AppendInt32Field(b, "itemID", slot.itemID)
	b = protocol.AppendStringField(b, "SlotName", slot.slotName)
	b = protocol.AppendStringField(b, "Headline", slot.headline)
	b = protocol.AppendStringField(b, "Description", slot.description)
	b = protocol.AppendStringField(b, "ItemType", slot.itemType)
	b = protocol.AppendInt32Field(b, "Position", slot.position)
	b = protocol.AppendInt32Field(b, "ItemTier", slot.itemTier)
	b = protocol.AppendBoolField(b, "bIsOwned", true)
	b = protocol.AppendBoolField(b, "bIsEquipped", state == "equipped")
	b = protocol.AppendBoolField(b, "bCanEquip", true)
	b = protocol.AppendBoolField(b, "bIsPreview", state == "preview")
	b = protocol.AppendBoolField(b, "bIsPlaceholder", slot.itemID == 0)
	b, stack = protocol.AppendObjectEnd(b, stack)
	return b, stack
}

func appendMmogNamedLoadoutDetail(b []byte, stack []int, name string, loadout mmogShipLoadoutSeed) ([]byte, []int) {
	b, stack = protocol.AppendObjectStart(b, stack, name)
...
func appendMmogLoadoutDetailContent(b []byte, stack []int, loadout mmogShipLoadoutSeed) ([]byte, []int) {
	...
	b = protocol.AppendInt32Field(b, "LoadoutID", loadout.loadoutID())
	b = protocol.AppendInt32Field(b, "ShipID", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "weaponCount", int32(len(loadout.weaponSlots())))
	b = protocol.AppendInt32Field(b, "moduleCount", int32(len(loadout.abilitySlots())))
	b = protocol.AppendInt32Field(b, "officerBriefingCount", int32(len(loadout.perkSlots())))
	b, stack = protocol.AppendArrayStart(b, stack, "weapons")
	for _, slot := range loadout.weaponSlots() {
		b, stack = appendMmogLoadoutDetailSlotEntry(b, stack, loadout, slot)
	}
	b, stack = protocol.AppendObjectEnd(b, stack)
	b, stack = protocol.AppendArrayStart(b, stack, "modules")
	for _, slot := range loadout.abilitySlots() {
		b, stack = appendMmogLoadoutDetailSlotEntry(b, stack, loadout, slot)
	}
	b, stack = protocol.AppendObjectEnd(b, stack)
	b, stack = protocol.AppendArrayStart(b, stack, "officerBriefings")
...
func appendMmogLoadoutDetailSlotEntry(b []byte, stack []int, loadout mmogShipLoadoutSeed, slot mmogLoadoutItemSeed) ([]byte, []int) {
	...
	b = protocol.AppendStringField(b, "Headline", slot.headline)
	b = protocol.AppendStringField(b, "Description", slot.description)
	b = protocol.AppendStringField(b, "IconImagePath", "")
	b = protocol.AppendStringField(b, "ItemType", slot.itemType)
	b = protocol.AppendInt32Field(b, "LoadoutID", loadout.loadoutID())
	b = protocol.AppendInt32Field(b, "ShipID", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "ItemID", slot.itemID)
...
func appendMmogOwnedInventoryItem(b []byte, stack []int, item mmogInventoryItemSeed) ([]byte, []int) {
	b, stack = protocol.AppendUnnamedObjectStart(b, stack)
	b = protocol.AppendStringField(b, "ID", item.id)
	b = protocol.AppendStringField(b, "Name", item.name)
	b = protocol.AppendStringField(b, "Type", inventoryDisplayType(item.itemType))
	b = protocol.AppendInt32Field(b, "ItemID", item.itemID)
	b = protocol.AppendInt32Field(b, "itemID", item.itemID)
	b = protocol.AppendStringField(b, "ExternalID", item.externalID)
	b = protocol.AppendStringField(b, "Description", item.description)
	if item.shipID != 0 {
		b = protocol.AppendInt32Field(b, "ShipID", item.shipID)
	}
	if item.loadoutID != 0 {
		b = protocol.AppendInt32Field(b, "LoadoutID", item.loadoutID)
	}
	if item.slotName != "" {
		b = protocol.AppendStringField(b, "SlotName", item.slotName)
	}
	if item.manufacturer != "" {
		b = protocol.AppendStringField(b, "Manufacturer", item.manufacturer)
	}
	b = protocol.AppendBoolField(b, "bIsOwned", true)
	b = protocol.AppendBoolField(b, "bIsEquipped", item.itemType != "ship")
	b = protocol.AppendBoolField(b, "bIsPreviewable", true)
	b = protocol.AppendInt32Field(b, "Quantity", item.quantity)
	b, stack = protocol.AppendObjectEnd(b, stack)
	return b, stack
}
...
func appendMmogSetLoadoutShipEntry(b []byte, stack []int, fleetType int32, loadout mmogShipLoadoutSeed) ([]byte, []int) {
	b, stack = protocol.AppendUnnamedObjectStart(b, stack)
	b = protocol.AppendStringField(b, "shipName", loadout.ship.name)
	b = protocol.AppendStringField(b, "m_shipName", loadout.ship.name)
	b = protocol.AppendInt32Field(b, "shipID", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "shipId", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "m_shipId", loadout.ship.id)
	b = protocol.AppendInt32Field(b, "shipClass", loadout.ship.shipClass)
	b = protocol.AppendInt32Field(b, "m_shipClass", loadout.ship.shipClass)
	b = protocol.AppendInt32Field(b, "FleetType", fleetType)
	b = protocol.AppendInt32Field(b, "m_fleetType", fleetType)
	b = protocol.AppendInt32Field(b, "shipXP", 0)
	b = protocol.AppendInt32Field(b, "m_shipXp", 0)
	b = protocol.AppendInt32Field(b, "m_shipTier", 1)
	b = protocol.AppendInt32Field(b, "m_manufacturerId", 0)
	b = protocol.AppendStringField(b, "m_shipClassImagePath", "")
	b = protocol.AppendStringField(b, "m_shipClassIconPath", "")
	b = protocol.AppendStringField(b, "m_tierIconImagePath", "")
	b = protocol.AppendInt32Field(b, "loadoutCount", 1)
	b, stack = protocol.AppendArrayStart(b, stack, "loadouts")
	b, stack = appendMmogUnnamedLoadoutDetail(b, stack, loadout)
	b, stack = protocol.AppendObjectEnd(b, stack)
	b, stack = protocol.AppendArrayStart(b, stack, "m_loadouts")
...
func appendMmogBattleReadyFleetEntry(b []byte, stack []int, fleet mmogFleetSeed) ([]byte, []int) {
	b, stack = protocol.AppendUnnamedObjectStart(b, stack)
	b = protocol.AppendStringField(b, "FleetID", fleet.token)
	b = protocol.AppendStringField(b, "FleetName", fleet.displayName)
	b = protocol.AppendInt32Field(b, "FleetType", fleet.fleetType)
	b = protocol.AppendInt32Field(b, "FlagShipID", fleet.flagshipShipID)
	b = protocol.AppendInt32Field(b, "FlagShipLoadoutID", fleet.flagshipLoadoutID)
	b = protocol.AppendBoolField(b, "bIsBattleReady", fleet.active && len(fleet.shipLoadouts) > 0)
	b = protocol.AppendInt32Field(b, "BonusCount", 1)
	b, stack = protocol.AppendArrayStart(b, stack, "Bonuses")
	b, stack = protocol.AppendUnnamedObjectStart(b, stack)
	b = protocol.AppendStringField(b, "SlotName", "BattleReadyBonus")
	b = protocol.AppendInt32Field(b, "BonusID", 0)
	b = protocol.AppendBoolField(b, "bIsOwned", true)
	b = protocol.AppendBoolField(b, "bIsPlaceholder", true)
	b, stack = protocol.AppendObjectEnd(b, stack)
	b, stack = protocol.AppendObjectEnd(b, stack)
	b, stack = protocol.AppendObjectEnd(b, stack)
	return b, stack
